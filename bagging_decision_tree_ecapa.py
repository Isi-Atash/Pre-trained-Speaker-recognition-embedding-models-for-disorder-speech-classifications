...
scores_H = cross_val_score(DecisionTreeClassifier(), X_hun, y_hun, cv=10)
print('SVC H Accuracy: %0.2f (+/- %0.2f)' % (scores_H.mean(), scores_H.std() * 2))

scores_D = cross_val_score(DecisionTreeClassifier(), X_dutch, y_dutch, cv=10)
print('SVC D Accuracy: %0.2f (+/- %0.2f)' % (scores_D.mean(), scores_D.std() * 2))

scores_H_D = cross_val_score(DecisionTreeClassifier(), X, y, cv=10)
...
#predict
y_pred = bag_model.predict(X_dutch)
print('Bagging Predicted values: ', y_pred)
#accuracy
from sklearn.metrics import accuracy_score
print('Bagging Prediction Accuracy: ', accuracy_score(y_dutch, y_pred))
#
